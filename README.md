...
| --- | --- | --- |
| 01 | 取り込みと素性調査 | **できた** |
| 02 | クレンジング(欠損・表記ゆれ・型・税区分) | **できた** |
| 03 | 名寄せと結合 | **できた** |
| 04 | 集計とサマリ | **できた** |
| 05 | 出力と再実行(冪等) | **できた** |

生ログ36行が、月ごとに差し替えられる日次サマリの Parquet になるまでを通しで作ります。
5章を終えると `load_raw` / `clean_raw` / `join_master` / `summarize` の4関数が手元に残り、
それが `quest-01` で自力実装するときの下敷きになります。
