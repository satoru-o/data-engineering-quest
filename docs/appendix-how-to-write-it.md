...
やることは分かっているのにコードにならない、という状態のための章です。
データエンジニアリングの話ではなく、**手の動かし方**の話をします。
...
多くの場合、根っこにあるのは3番目です。
**書けないというより、書いたものを確かめる速度が遅い**という状態です。
まずここから直すと、他の2つも楽になります。

---

## 1. 対話的にやる。スクリプトは最後に書く

いちばん効くのがこれです。ファイルに書いて実行、を繰り返すのは避けます。

```bash
./repl.sh
```

IPython が立ち上がり、`pd` などが読み込まれた状態で始まります。ここで1行ずつ試します。
...
**動いた行だけを `work/ingest.py` に写します。** これが基本の進め方です。
「書く → 実行 → エラー」ではなく「試す → 動いた → 記録する」の順にします。

REPL では変数が残るので、途中からやり直せます。
1200行の読み込みを毎回待つ必要もありません。

### スクリプトの途中から対話に落ちる

書き途中のコードの真ん中で止めて、その場の変数を触ることもできます。
...
`breakpoint()` を置いて `./run.sh` を実行します。その時点の `df` を自由に触れます。
`c` で続行、`q` で終了です。print 文を仕込むより速く原因にたどり着けます。

---

## 2. 日本語で手順を書いてから、1行ずつ埋める

やりたいことを日本語のまま、コメントで並べます。
...
quest-01 なら `spec/orders.md` の「処理」節がそのまま設計書になっています。**写すだけで大丈夫です。**

ここから、**書ける行だけ**埋めていきます。書けない行は飛ばして次に進みます。
...
7つ全部が埋まらないと動かない、ということはありません。
**常に「動く状態」を保ったまま、1つずつ埋めていきます。**

出力の行数が合わなくても、型が違っても、
`out/report.txt` が「どこまで来たか」を返してくれます。1行埋めるごとに `./run.sh` してかまいません。

---

## 3. 語彙表 — 日本語のやりたいこと → 検索語

**検索できないのは、名前を知らないからです。** ここが最初の壁になります。
「pandas やりたいこと」で検索しても目的のものは出てきませんが、
「pandas drop_duplicates」なら公式ドキュメントにすぐ着きます。
...
**まず `unique()` を叩くのがおすすめです。** 汚れたデータを扱うときに、いちばん役に立ちます。
...
「この列にどんな汚れがあるか」を推測しなくて済みます。**全部見えます。**
仕様書を読んで想像するより、データに聞くほうが速くて確実です。
...
思い出せそうな断片(`dup`, `sort`, `merge`, `na`)を入れて絞り込みます。
検索エンジンを開くより速いことが多いです。

文字列操作なら `df["col"].str` の下を見ます。
...
IPython なら、末尾に `?` を付けると説明が出ます。

```python
In [6]: df.drop_duplicates?
```

`??` ならソースまで表示されます。ブラウザに移らなくて済むので、作業の流れが切れません。

### タブ補完

`df.dr` まで打って `Tab` を押すと候補が出ます。
**名前を全部覚える必要はありません。頭の2〜3文字だけで十分です。**

---

## 5. 検索の作法

- **英語で検索する。** 日本語の記事は数が少なく、内容が古いこともあります。
  `pandas drop duplicates keep last` のように、単語を並べるだけで大丈夫です
- **公式ドキュメントを優先する。** `pandas.pydata.org` が候補に出たら、それを開きます。
  個人ブログには pandas 1.x 時代の書き方が残っていることがあります
- **「たぶんある」と思って探す。** pandas は API が広いので、
  自分で `for` ループを書き始めたときは、探し方を変えてみると見つかることが多いです
- **エラーは最後の行から読む。** 上のスタックトレースはライブラリの内部なので、
  最終行の `ValueError: ...` を先に読みます

```
ValueError: invalid literal for int() with base 10: 'NULL'
```

この1行に「int にしようとした」「`'NULL'` という文字列で失敗した」が書かれています。
**エラーメッセージは、検索するより先に読んでみてください。**

---

## 6. AIに聞くときの聞き方

聞くこと自体は良い方法です。**聞き方によって、身に付き方が変わります。**
...
3. REPL で、引数を変えて動かしてみる

ここまでやると、次からは自分で書けるようになります。
写して次に進むだけだと、次も同じところで止まりやすいです。
**答えではなく語彙をもらう**、という使い方が効きます。
...
**特に行数です。** 意図せず行が減っていた、増えていた、というのは
データ処理でよく起こることです。
...
**減った数に説明が付くか**を毎回確認します。
「なんとなく減った」を通してしまうと、あとで数字が合わなくなったときに、
どこが原因か追いにくくなります。
...
1200行でいきなり試さず、**20行で挙動を確かめてから**全体に適用します。
...
さらに小さく、値1個で試すこともできます。

```python
In [12]: parse_amount("￥1,200")
```

関数が正しいかを、DataFrame を通さずに確かめられます。
**「関数が悪いのか、適用の仕方が悪いのか」を切り分ける**のが目的です。
...
この流れでは、**推測をしていません。**
毎回データに聞いて、返ってきたものを見てから次を決めています。
これができるようになると、知らないライブラリでも進めるようになります。
